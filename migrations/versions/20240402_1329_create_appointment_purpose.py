...
def downgrade() -> None:
    op.execute(
        "drop trigger if exists appointment_purpose_updated_at_trg on public.appointment_purposes;"
    )
    op.execute(
        "drop trigger if exists appointment_purpose_deleted_at_trg on public.appointment_purposes;"
    )
