...
class MedicinesPrescriptionDBModel(BaseDBModel):
    ...
    appointment_purpose_id = Column(
        BigInteger, ForeignKey("public.appointment_purposes.id"), nullable=False
    )

    drug_id = Column(
        BigInteger, ForeignKey("public.drugs.id", ondelete="CASCADE"), nullable=False
    )
