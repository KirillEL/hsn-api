...
class SuppliedDiagnosesDBModel(BaseDBModel):
    ...
    diagnose_catalog_id = Column(
        BigInteger, ForeignKey("public.diagnoses_catalog.id"), nullable=False
    )
    medicine_prescription_id = Column(
        BigInteger, ForeignKey("public.medicine_prescriptions.id"), nullable=False
    )
