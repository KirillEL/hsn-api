...
class ClinicalAssesmentDBModel(BaseDBModel):
    ...
    distance_walking_6_minutes = Column(
        PGEnum(*distance_walkings, name="distance_walking_type", create_type=True),
        nullable=False,
    )
    ...
    has_decreased_exercise_tolerance = Column(
    ...
    patient_appointment_id = Column(
        BigInteger,
        ForeignKey("public.appointments.id", ondelete="CASCADE"),
        nullable=False,
    )
    patient_hospitalization_id = Column(
        BigInteger,
        ForeignKey("public.patient_hospitalizations.id", ondelete="CASCADE"),
        nullable=False,
    )
    patient_id = Column(
        BigInteger, ForeignKey("public.patients.id", ondelete="CASCADE"), nullable=False
    )
