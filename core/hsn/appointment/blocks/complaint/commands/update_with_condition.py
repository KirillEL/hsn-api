...
async def hsn_command_block_complaint_and_clinical_condition_update(
    doctor_id: int, context: HsnCommandBlockComplaintAndClinicalConditionUpdateContext
) -> AppointmentComplaintWithClinicalCondition:
    ...
    if not appointment:
        raise NotFoundException(
            "Прием c id:{} не найден".format(context.appointment_id)
        )
    ...
    # 1
    query_get_block_complaint_id: Select = select(
        AppointmentDBModel.block_complaint_id
    ...
    query_get_block_clinical_condition_id: Select = select(
    ...
    if not block_clinical_condition_id:
        raise NotFoundException(
            message="У приема c id:{} не привязан еще блок клинического состояния!".format(
    ...
    if payloads["block_clinical_condition"]:
        query_update_block_clinical_condition: ReturningUpdate = (
            ...
            .where(
                AppointmentClinicalConditionBlockDBModel.id
                == block_clinical_condition_id
            )
