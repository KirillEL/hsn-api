...
async def hsn_query_block_laboratory_test_fields():
    ...
    categories = {
        ...
        "blood_chemistry": ["lpnp", "general_hc", "natriy", "kaliy", "glukoza", "mochevaya_kislota", "skf", "kreatinin",
                            "tg", "lpvp"],
        "general_urine_analysis": ["protein", "urine_eritrocit", "urine_leycocit", "microalbumuria"]
    }
    ...
    for category_name, fields in categories.items():
        ...
        if category_name == "hormonal_blood_analysis":
            ...
            for field, individual_date in zip(fields, date_name):
                if field in display_names and field in all_columns and individual_date in all_columns:
            ...
            shared_date = date_name if isinstance(date_name, str) and date_name in all_columns else None

            # Only proceed if shared_date is not None
            if shared_date:
                field_objects = [
                    BaseTextField(textName=field, displayName=display_names[field])
                    for field in fields if field in display_names and field in all_columns
                ]

                category_data = None
                if category_name == "general_blood_analysis":
                    category_data = GeneralBloodAnalysisTextDateField(fields=field_objects, dateName=shared_date)
                elif category_name == "blood_chemistry":
                    category_data = BloodChemistryTextDateField(fields=field_objects, dateName=shared_date)
                elif category_name == "general_urine_analysis":
                    category_data = GeneralUrineAnalysisTextDateField(fields=field_objects, dateName=shared_date)
